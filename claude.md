...
The monitor polls a dashboard endpoint for pending score-correction commands
and applies them into AES over the existing port-17471 connection. **The full
endpoint contract (`GET`/`POST .../outbox`) — the spec for the dashboard repo
(aes-tourney-director) to implement — lives in `docs/DASHBOARD_OUTBOX_API.md`,
not in this file.** Scope: score/outcome corrections only (matches
`RemoteEntryUpdateType.MatchData`) — no work-team reassignment, no
seed/bracket edits. See `docs/WRITE_BACK_EDITOR_SCOPING.md` for why this
approach was chosen over a `.vsf` file editor.
